...
public class FrequencyMasterResourceImpl extends AbstractResourceImpl implements FrequencyMasterResource {
	...
	public ResponseEntity<Response> add(FrequencyMasterRequestDTO requestDTO) {
		...
		frequencyMaster = frequencyMasterService.add(frequencyMaster);

		if (frequencyMaster == null) {
			return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
					.body(ResponseBuilder.builder().status(StatusEnum.FAILURE.getValue(),
							ErrorCode.SERVICE_UNAVAILABLE.getCode(), "Frequency not added").build());
		}
		return ResponseEntity.status(HttpStatus.CREATED).body(ResponseBuilder.builder()
				.status(StatusEnum.SUCCESS.getValue(), SuccessCode.OK.getCode(), "Frequency added successfully.")
				.build());
	...
	public ResponseEntity<Response> getAll() {
		...
		List<FrequencyMasterResponseDTO> responseDTOList = frequencyMasterDTOMapper
				.entityListToDtoList(frequencyMasterList);
					...
					.status(HttpStatus.SERVICE_UNAVAILABLE).body(
							ResponseBuilder
									.builder().status(StatusEnum.FAILURE.getValue(),
											ErrorCode.SERVICE_UNAVAILABLE.getCode(), "Frequency records not found")
									.build());
		}
		return ResponseEntity.status(HttpStatus.OK).body(ResponseBuilder.builder().status(StatusEnum.SUCCESS.getValue(),
				SuccessCode.OK.getCode(), "Frequency records retrieved successfully").results(responseDTOList).build());
	}
